...
import jax
from sdfs import ramp_sdf, box_sdf, capsule_sdf
...
def _translate(
    ...
    cos_t = jnp.cos(angle)
    sin_t = jnp.sin(angle)
    transformed = jnp.matmul(jnp.linalg.inv(
        jnp.array(
            [
                [cos_t, -sin_t, x],
                [sin_t, cos_t, y],
...
def _scene_sdf_from_tilemap(tilemap: jax.Array, p: jax.Array, floor_level=-3.0):
    ...
    tile_dists = jax.vmap(lambda i: process_tile(p, i // w, i % w, tilemap[i // w, i % w]))(dists)
