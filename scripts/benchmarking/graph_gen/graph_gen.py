...
def plot_cpu_gpu_render_frame(output_dir: str, y_axis: str, log: bool) -> None:
    os.makedirs(output_dir, exist_ok=True)

    input_dir = "scripts/benchmarking/graph_gen/jsons"
    
    cpu_empty_world_json = "render_frame_cpu_1_agent_empty_world_100_steps.json"
    cpu_non_empty_world_json = "render_frame_cpu_1_agent_non_empty_world_simple_100_steps.json"
    gpu_empty_world_json = "render_frame_gpu_1_agent_empty_world_100_steps.json"
    gpu_non_empty_world_json = "render_frame_gpu_1_agent_2x2_100_steps.json"
    ...
    # Assume that there is only one scenario in both json files
    cpu_empty_world_metrics_list = list(map(lambda inner_list: inner_list[1], list(cpu_empty_world_results.items())[0][1]))
    cpu_non_empty_world_metrics_list = list(map(lambda inner_list: inner_list[1], list(cpu_non_empty_world_results.items())[0][1]))
    gpu_empty_world_metrics_list = list(map(lambda inner_list: inner_list[1], list(gpu_empty_world_results.items())[0][1]))
    gpu_non_empty_world_metrics_list = list(map(lambda inner_list: inner_list[1], list(gpu_non_empty_world_results.items())[0][1]))
    scenario_name = "CPU vs GPU"
    ...
    if (y_axis == "time"):
      ...
      plt.ylabel(f"Total rendered frame per second")
      plt.title(f"Total Rendered Frame per Second (100 steps) vs Batch Size ({scenario_name})")
      plot_path = os.path.join(
        output_dir, f"{scenario_name}_total-steps-per-sec.png"
      )
...
def plot_agents_render_frame_graph(output_dir: str, y_axis: str, log: bool) -> None:
    ...
    if (y_axis == "time"):
      plt.ylabel("Total time (in s)")
      plt.title(f"Total time (100 steps) vs Batch Size (vary by # agents)")
      plot_path = os.path.join(
          output_dir, f"{scenario_name}_total-time.png"
      )
    else:
      plt.ylabel(f"Total frames rendered per second")
      plt.title(f"Total Frames Rendered per Second (100 steps) vs Batch Size (vary by # agents)")
      plot_path = os.path.join(
        output_dir, f"{scenario_name}_total-steps-per-sec.png"
      )
...
def plot_grid_size_render_graph_step(output_dir: str, y_axis: str, log: bool) -> None:
    ...
    gpu_2x2_world_json = "render_frame_gpu_1_agent_2x2_100_steps.json"
    gpu_4x4_world_json = "render_frame_gpu_1_agent_4x4_100_steps.json"
    gpu_8x8_world_json = "render_frame_gpu_1_agent_8x8_100_steps.json"
    ...
    _, gpu_2x2_world_metrics_list = list(gpu_2x2_world_results.items())[0]
    _, gpu_4x4_world_metrics_list = list(gpu_4x4_world_results.items())[0]
    _, gpu_8x8_world_metrics_list = list(gpu_8x8_world_results.items())[0]
    scenario_name = "vary by grid sizes"
    ...
    gpu_8x8_world_metrics_list = [m[1] for m in gpu_8x8_world_metrics_list]
    ...
    for x_val, pu in special_points:
        if pu == "2":
            y_val = gpu_2x2_world_steps[gpu_2x2_world_batch_sizes.index(x_val)]
            color = "red"
        elif pu == "4":
            y_val = gpu_4x4_world_steps[gpu_4x4_world_batch_sizes.index(x_val)]
            color = "green"
        elif pu == "8":
            y_val = gpu_8x8_world_steps[gpu_8x8_world_batch_sizes.index(x_val)]
            color = "blue"
        else:
            raise ValueError(f"Invalid value for pu: {pu}")
    ...
    if y_axis == "time":
        ...
        plt.ylabel(f"Total frames rendered per second")
        plt.title(f"Total Frames Rendered per Second (100 steps) vs Batch Size ({scenario_name})")
        plot_path = os.path.join(
            output_dir, f"{scenario_name}_total-steps-per-sec.png"
        )
...
def plot_grid_size_render_vs_step_graph(output_dir: str, y_axis: str, log: bool) -> None:
    os.makedirs(output_dir, exist_ok=True)

    input_dir = "scripts/benchmarking/graph_gen/jsons"
    gpu_2x2_step_json = "step_gpu_1_agent_simple_non_empty_world_100_steps.json"
    gpu_4x4_step_json = "step_gpu_1_agent_simple_non_empty_world_4x4_100_steps.json"
    gpu_8x8_step_json = "step_gpu_1_agent_simple_non_empty_world_8x8_100_steps.json"
    gpu_2x2_render_json = "render_frame_gpu_1_agent_2x2_100_steps.json"
    gpu_4x4_render_json = "render_frame_gpu_1_agent_4x4_100_steps.json"
    gpu_8x8_render_json = "render_frame_gpu_1_agent_8x8_100_steps.json"

    gpu_2x2_step_json = os.path.join(input_dir, gpu_2x2_step_json)
    gpu_4x4_step_json = os.path.join(input_dir, gpu_4x4_step_json)
    gpu_8x8_step_json = os.path.join(input_dir, gpu_8x8_step_json)
    gpu_2x2_render_json = os.path.join(input_dir, gpu_2x2_render_json)
    gpu_4x4_render_json = os.path.join(input_dir, gpu_4x4_render_json)
    gpu_8x8_render_json = os.path.join(input_dir, gpu_8x8_render_json)

    with open(gpu_2x2_step_json, "r") as file:
        gpu_2x2_step_data = json.load(file)
    with open(gpu_4x4_step_json, "r") as file:
        gpu_4x4_step_data = json.load(file)
    with open(gpu_8x8_step_json, "r") as file:
        gpu_8x8_step_data = json.load(file)
    with open(gpu_2x2_render_json, "r") as file:
        gpu_2x2_render_data = json.load(file)
    with open(gpu_4x4_render_json, "r") as file:
        gpu_4x4_render_data = json.load(file)
    with open(gpu_8x8_render_json, "r") as file:
        gpu_8x8_render_data = json.load(file)
    
    gpu_2x2_step_results = gpu_2x2_step_data['benchmark_results']
    gpu_4x4_step_results = gpu_4x4_step_data['benchmark_results']
    gpu_8x8_step_results = gpu_8x8_step_data['benchmark_results']
    gpu_2x2_render_results = gpu_2x2_render_data['benchmark_results']
    gpu_4x4_render_results = gpu_4x4_render_data['benchmark_results']
    gpu_8x8_render_results = gpu_8x8_render_data['benchmark_results']
    
    _, gpu_2x2_step_metrics_list = list(gpu_2x2_step_results.items())[0]
    _, gpu_4x4_step_metrics_list = list(gpu_4x4_step_results.items())[0]
    _, gpu_8x8_step_metrics_list = list(gpu_8x8_step_results.items())[0]
    _, gpu_2x2_render_metrics_list = list(gpu_2x2_render_results.items())[0]
    _, gpu_4x4_render_metrics_list = list(gpu_4x4_render_results.items())[0]
    _, gpu_8x8_render_metrics_list = list(gpu_8x8_render_results.items())[0]
    scenario_name = "step vs render_frame"

    gpu_2x2_render_metrics_list = [m[1] for m in gpu_2x2_render_metrics_list]
    gpu_4x4_render_metrics_list = [m[1] for m in gpu_4x4_render_metrics_list]
    gpu_8x8_render_metrics_list = [m[1] for m in gpu_8x8_render_metrics_list]

    gpu_2x2_step_batch_sizes = [m['batch_size'] for m in gpu_2x2_step_metrics_list]
    gpu_4x4_step_batch_sizes = [m['batch_size'] for m in gpu_4x4_step_metrics_list]
    gpu_8x8_step_batch_sizes = [m['batch_size'] for m in gpu_8x8_step_metrics_list]
    gpu_2x2_render_batch_sizes = [m['batch_size'] for m in gpu_2x2_render_metrics_list]
    gpu_4x4_render_batch_sizes = [m['batch_size'] for m in gpu_4x4_render_metrics_list]
    gpu_8x8_render_batch_sizes = [m['batch_size'] for m in gpu_8x8_render_metrics_list]

    if y_axis == "time":
        gpu_2x2_step_steps = [m['total_time'] for m in gpu_2x2_step_metrics_list]
        gpu_4x4_step_steps = [m['total_time'] for m in gpu_4x4_step_metrics_list]
        gpu_8x8_step_steps = [m['total_time'] for m in gpu_8x8_step_metrics_list]
        gpu_2x2_render_steps = [m['total_time'] for m in gpu_2x2_render_metrics_list]
        gpu_4x4_render_steps = [m['total_time'] for m in gpu_4x4_render_metrics_list]
        gpu_8x8_render_steps = [m['total_time'] for m in gpu_8x8_render_metrics_list]
    elif y_axis == "steps":
        gpu_2x2_step_steps = [m['num_steps'] * m['batch_size'] / m['total_time'] for m in gpu_2x2_step_metrics_list]
        gpu_4x4_step_steps = [m['num_steps'] * m['batch_size'] / m['total_time'] for m in gpu_4x4_step_metrics_list]
        gpu_8x8_step_steps = [m['num_steps'] * m['batch_size'] / m['total_time'] for m in gpu_8x8_step_metrics_list]
        gpu_2x2_render_steps = [m['num_steps'] * m['batch_size'] / m['total_time'] for m in gpu_2x2_render_metrics_list]
        gpu_4x4_render_steps = [m['num_steps'] * m['batch_size'] / m['total_time'] for m in gpu_4x4_render_metrics_list]
        gpu_8x8_render_steps = [m['num_steps'] * m['batch_size'] / m['total_time'] for m in gpu_8x8_render_metrics_list]
    else:
        raise ValueError(f"Invalid value for y_axis: {y_axis}")

    plt.figure()
    plt.plot(gpu_2x2_step_batch_sizes, gpu_2x2_step_steps, color="red", marker="x", label="step (2x2 world)")
    plt.plot(gpu_4x4_step_batch_sizes, gpu_4x4_step_steps, color="green", marker="x", label="step (4x4 world)")
    plt.plot(gpu_8x8_step_batch_sizes, gpu_8x8_step_steps, color="blue", marker="x", label="step (8x8 world)")
    plt.plot(gpu_2x2_render_batch_sizes, gpu_2x2_render_steps, color="red", marker="o", label="render_frame (2x2 world)")
    plt.plot(gpu_4x4_render_batch_sizes, gpu_4x4_render_steps, color="green", marker="o", label="render_frame (4x4 world)")
    plt.plot(gpu_8x8_render_batch_sizes, gpu_8x8_render_steps, color="blue", marker="o", label="render_frame (8x8 world)")

    legend_elements = [
      Line2D([0], [0], color='black', lw=1, marker='x', label='step'),
      Line2D([0], [0], color='black', lw=1, marker='o', label='render_frame'),
      mpatches.Patch(color='red', label='2x2 world'),
      mpatches.Patch(color='green', label='4x4 world'),
      mpatches.Patch(color='blue', label='8x8 world'),
    ]

    plt.legend(handles=legend_elements, loc='upper left')


    special_points = [(2**7, '2'), (2**4, '4'), (2**3, '8')]

    # Special points
    for x_val, pu in special_points:
        if pu == "2":
            y_val = gpu_2x2_render_steps[gpu_2x2_render_batch_sizes.index(x_val)]
            color = "red"
        elif pu == "4":
            y_val = gpu_4x4_render_steps[gpu_4x4_render_batch_sizes.index(x_val)]
            color = "green"
        elif pu == "8":
            y_val = gpu_8x8_render_steps[gpu_8x8_render_batch_sizes.index(x_val)]
            color = "blue"
        else:
            raise ValueError(f"Invalid value for pu: {pu}")
...
if __name__ == "__main__":
  ...
  #   True
  # )
  plot_grid_size_render_vs_step_graph(
     "scripts/benchmarking/graph_gen/graphs",
     "steps",
     True,
	)