...
SCENARIOS = {
    ...
    # In the future, add additional scenarios here. E.g.,
    # "vary_agent_count": {
...
# --------------------------------------------------------------------------------------
def measure_compile_time(key, icland_params, init_state, batched_step, warmup_steps):
    """
    ...
    for _ in range(num_steps):
        ...
        if gpu_available:
            for i in range(num_gpus):
                ...
                max_gpu_usage_percent[i] = max(max_gpu_usage_percent[i], gpu_util_percent)
                max_gpu_memory_usage_mb[i] = max(max_gpu_memory_usage_mb[i], gpu_mem_usage_mb)
...
# --------------------------------------------------------------------------------------
def run_scenario_benchmark(scenario_name, scenario_def):
    """
    ...
    if "batch_sizes" in params and len(benchmark_runs) > 0:
        ...
        if has_gpu:
            max_gpu_usage_list = [run["max_gpu_usage_percent"] for run in benchmark_runs]
            # Number of GPUs from first non-empty
            num_gpus = len(next(r["max_gpu_usage_percent"] for r in benchmark_runs if len(r["max_gpu_usage_percent"])>0))
...
# --------------------------------------------------------------------------------------
def run_all_scenarios(scenarios_dict=None):
